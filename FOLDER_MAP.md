...
├── index.html             🔗 첫 접속 시 sites/3dprinter/index.html로 자동 연결해 주는 리다이렉션 파일
├── .gitignore            git이 무시할 파일 목록 (임시파일 등)
│
├── inbox/                 📥 미가공 외부 자료 보관용 인박스 (.gitignore 설정으로 깃허브 배포 제외)
│
├── docs/                 📄 프로젝트 문서 보관
...
└── sites/                🌐 사업부별 사이트들이 사는 곳
    ...
    │   ├── js/                스크립트 폴더
    │   │   ├── support.js        (현재 페이지 렌더용)
    │   │   └── image-slot.js     (동상)
    │   ├── images/            이 사이트 사진 및 조형 GIF 파일들
    │   └── uploads/           영상 등 큰 파일
    │
    ├── cnc/                ⬜ CNC 사업부 (예정 — 빈 골격)
...
| **미가공 PPTX, 초안 데이터 등 외부에서 유입된 소스** | `inbox/` (로컬 전용) |
| 프로젝트 설명·이력 문서 | `docs/` |
...
| `sites/marking/` | `marking.cscam.co.kr` |
